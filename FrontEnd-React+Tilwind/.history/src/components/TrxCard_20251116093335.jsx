...
const TrxCard = ({trx}) => {
    ...
    // data or irtime purchse 
    const {phone_number,network, data_plan} = trx;

    const getNetworkLogo = (net) => {
        let networkName = net.toLowerCase();
        return (networkName === 'mtn') ? mtnLogo :
                (networkName === 'airtel') ? airtelLogo :
                (networkName === 'glo') ? gloLogo :
                (networkName === '9mobile') ? nineMobileLogo : null
    }
            ...
            <div className="flex items-center justify-between p-2 bg-gray-50 rounded-lg cursor-pointer transform transition-all duration-300 hover:bg-white hover:shadow-lg hover:scale-[1.02] active:scale-[0.98]">
                  <div className="flex items-center space-x-4">
                    {/* <div className="h-12 w-12 rounded-full bg-purple-100 flex items-center justify-center"> */}
                      <img className="h-12 w-12 rounded-full bbd" src={getNetworkLogo(network)} alt={network} className="h-6 w-6"/>
                    {/* </div> */}
                    <div>
            ...
            <div className="flex items-center justify-between p-2  bg-gray-50 rounded-lg cursor-pointer transform transition-all duration-300 hover:bg-white hover:shadow-lg hover:scale-[1.02] active:scale-[0.98]">
                  <div className="flex items-center space-x-4">
                    <div className="h-12 w-12 rounded-full bg-indigo-100 flex items-center justify-center">
                      <img src={getNetworkLogo(network)} alt={network} className="h-6 w-6"/>
                    </div>
                    <div>
                      <div className="text-sm font-medium text-gray-900">Data Purchase</div>
                      <div className="flex items-center space-x-2 mt-1">
                        <span className="text-xs text-gray-500">{data_plan} : {phone_number}</span>
                        <span className="text-xs text-gray-400">•</span>
                        <span className="text-xs text-gray-500">{getFormattedDate(trx_date)}</span>
                      </div>
                    </div>
