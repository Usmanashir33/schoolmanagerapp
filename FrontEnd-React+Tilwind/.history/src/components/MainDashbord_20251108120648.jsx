...
import DataSellingPage from './BuyData';
import AirtimeSellingPage from './BuyAirtime';
  ...
  useEffect(() => {
      const handleClickOutside = (event) => {
        if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
          setShowAccounts(false); // hide when clicked outside
        }
      };
            ...
            <div className="flex items-center justify-between h-16 px-6">
              ...
              <div className="flex items-center space-x-6">
                ...
                <div className="relative ">
                  ...
                  {/* dropdown here  */}
                  {showAccounts && 
                  <div  ref={dropdownRef} className="absolute top-7 w-full right-0  mt-2  bg-white border border-gray-200 rounded-md shadow-lg">
                    <ul className="">
                      <li>
                       {currentUser?.is_staff && 
                        <Link to={`/staffusers`}className="flex items-center px-3 py-2 text-gray-600  hover:bg-gray-50 border ">
