...
const AATrxs = () => {
  ...
  const applyFilters = () => {
        ...
        if (statusFilters.success) {
            filtered = filtered.filter(trx => trx?.status?.toLowerCa() === 'success');
            alert('success' )
        }
        if (statusFilters.pending) {
            filtered = filtered.filter(trx => trx?.status?.toLowerCa() === 'pending');
            alert('pending' )
        }
        if (statusFilters.cancelled ) {
            filtered = filtered.filter(trx => trx?.status?.toLowerCa() === 'cancelled');
        }
        if (statusFilters.failed) {
            filtered = filtered.filter(trx => trx?.status?.toLowerCa() === 'failed');
        }
        if (statusFilters.approved) {
            filtered = filtered.filter(trx => trx?.status?.toLowerCa() === 'approved');
        }
