...
const AATrxs = () => {
  ...
  const applyFilters = () => {
        ...
        if (statusFilters.success) {
            filtered = filtered.filter(trx => trx.status.to === 'success');
        }
        if (statusFilters.pending) {
            filtered = filtered.filter(trx => trx.status.to === 'pending');
        }
        if (statusFilters.failed) {
            filtered = filtered.filter(trx => trx.status.to === 'failed');
        }
        if (statusFilters.failed) {
            filtered = filtered.filter(trx => trx.status.to === 'cancelled');
        }
