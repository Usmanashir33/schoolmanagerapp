...
const AATrxs = () => {
  ...
  const applyFilters = () => {
        ...
        if (statusFilters.success) {
            filtered = filtered.filter(trx => trx.status.toLo === 'success');
        }
        if (statusFilters.pending) {
            filtered = filtered.filter(trx => trx.status.toLo === 'pending');
        }
        if (statusFilters.failed) {
            filtered = filtered.filter(trx => trx.status.toLo === 'failed');
        }
        if (statusFilters.failed) {
            filtered = filtered.filter(trx => trx.status.toLo === 'cancelled');
        }
