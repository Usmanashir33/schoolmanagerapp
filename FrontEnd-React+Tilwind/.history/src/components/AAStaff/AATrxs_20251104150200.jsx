...
const AATrxs = () => {
  ...
  const resetFilters = () => {
          ...
          setStatusFilters('');
          setTransactionType('');
          setFilteredTrxs(pageContent?.trxs?.results || []);
      };
  const applyFilters = () => {
        ...
        if (statusFilters.toLowerCase() === 'success') {
            filtered = filtered.filter(trx => trx?.status?.toLowerCase() === 'success');
        }
        if (statusFilters.toLowerCase() === 'pending') {
            filtered = filtered.filter(trx => trx?.status?.toLowerCase() === 'pending');
        }
        if (statusFilters.toLowerCase() === 'cancelled' ) {
            filtered = filtered.filter(trx => trx?.status?.toLowerCase() === 'cancelled');
        }
        if (statusFilters.toLowerCase() === 'failed') {
            filtered = filtered.filter(trx => trx?.status?.toLowerCase() === 'failed');
        }
        if (statusFilters.toLowerCase() === 'approved') {
            filtered = filtered.filter(trx => trx?.status?.toLowerCase() === 'approved');
        }
            ...
            <div className="p-4">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex space-x-3 relative ">
                   {(transactionType && transactionType !== 'All Types') && <span className="flex p-1 font-medium text-gray-900 items-center">
                    {filteredTrxs?.length}
                  </span>}
                  <div className="relative">
                    <button onClick={() => setIsFilterOpen(true)}  className="flex items-center space-x-2 px-2 py-2 border border-gray-300 rounded-lg text-gray-600 hover:bg-gray-50 cursor-pointer !rounded-button whitespace-nowrap">
                      <i className="fas fa-filter text-sm"></i>
                      <span  className="text-xs">{(transactionType || statusFilters)? `${transactionType}/${statusFilters}` :'All Types'}</span>
                  ...
                  {/* show filterd date  */} 
                      ...
                      <div className=" absolute -bottom-3 left-0 flex justify-between items-center text-xs font-medium text-blue">
                  ...
                  {isFilterOpen && (
                        <div className="absolute right-10 top-10 mt-2 w-72 bg-white rounded-lg shadow-lg z-50 border overflow-y-auto max-h-96">
                          <div className="p-4">
                            ...
                            {/* Status Type */}
                            <div className="mb-4">
                              <label className="block text-xs font-medium text-gray-700 mb-1">Status Types</label>
                              <div className="relative">
                                <select 
                                  className="w-full text-xs border rounded p-2 pr-8 appearance-none bg-white"
                                  value={statusFilters}
