...
const StaffPins = ({close,triggeredFunc,title=''}) => {
  ...
  const staffAuthUrl = "/staff/staff-auth/" ; 
  ...
  const handleProceedOtpChange = () => {
    let data = {staff_pin:newPin.join(''),staff_otp:otp, request_type:"reset"}
    sendArbitRequest(staffAuthUrl, "POST", data, handlePinResetServerResponse, true, false);
  }
    ...
    if (pinStep === "new") {
      ...
      if (confirmPin.filter(Boolean).length === 5) {
        if (newPin.join("") === confirmPin.join("")) {
          ...
          let staff_pin = newPin.join('') ;
          setFormdata({staff_pin})
          let data = {staff_pin,request_type:"request"}
          sendArbitRequest(staffAuthUrl, "POST", data, handlePinResetServerResponse, true, false);
          return ;
        } else {
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex ">
                ...
                <button
                  ...
                  onClick={() => {handleProceedOtpChange();}}
                >
                  Proceed
                </button>
              </div>
