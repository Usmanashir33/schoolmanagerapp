...
const AATrxs = () => {
  ...
  const applyFilters = () => {
        ...
        if (statusFilters.success) {
            filtered = filtered.filter(trx => trx.status.toLower() === 'success');
            alert('success' )
        }
        if (statusFilters.pending) {
            filtered = filtered.filter(trx => trx.status.toLower() === 'pending');
        }
        if (statusFilters.cancelled ) {
            filtered = filtered.filter(trx => trx.status.toLower() === 'cancelled');
        }
        if (statusFilters.failed) {
            filtered = filtered.filter(trx => trx.status.toLower() === 'failed');
        }
        if (statusFilters.approved) {
            filtered = filtered.filter(trx => trx.status.toLower() === 'approved');
        }
