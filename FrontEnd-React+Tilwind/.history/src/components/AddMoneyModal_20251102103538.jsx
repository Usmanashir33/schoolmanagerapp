        ...
        <div className="bg-white rounded-xl p-6 w-[90%] max-w-md">
          ...
          <div className="space-y-4  ">
             <div className="relative bg-gray-50 rounded-xl p-2 flex items-center cursor-pointer hover:bg-gray-100 transition-colors relative ">
              ...
              <div className="absolute  w-full bottom-0  border-gray-900 pointer-events-none">
                {currentUser?.account?.accountnumbers?.map((acc, index) => (
                  <div key={index} className="p-2 bg-white rounded-m mb-1 shadow-md border-b-1 border-gray-400">
                    <span className="text-sm text-gray-400 mt-1 block font-medium">
                      Acc Number: {acc.account_number}
                    </span>
                    <span className="text-sm text-opacity-80 mt-1 block font-medium">
                      Bank: {acc.bank_name}
                    </span>
                  </div>
                ))}
