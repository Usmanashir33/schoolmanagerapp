...
import { ChevronLeft, ChevronRight,CopyingIcon } from "lucide-react";
             ...
             <div className="relative bg-gray-50 rounded-xl p-2 flex items-center cursor-pointer hover:bg-gray-100 transition-colors relative ">
              ...
              <div className="absolute  w-full -bottom-100 z-10 rounded bg-gray-100 shadow-sm  border-gray-400 pointer-events-none">
                {currentUser?.account?.accountnumbers?.map((acc, index) => (
                  <div key={index} className="flex justify-between p-2 bg-white rounded-md mb-1  border-b-1 hover:shadow-md hover:border-gray-400 cursor-pointer transition-all animation-fadeIn">
                    <div>
                    ...
                    {/* <CopyingIcon className=" ml-auto text-gray-400 hover:text-black"
                      onClick={() => {
                        // copyToClipboard(`
                        //     Acc Number: ${currentUser?.account?.accountnumbers[0]?.account_number || 'null'}
                        //     Bank Name: ${currentUser?.account?.accountnumbers[0]?.bank_name || 'null'}
