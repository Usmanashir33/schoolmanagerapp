...
export default function AirtimeSellingPage() {
  const { setError,setSucces } = useContext(uiContext);
  ...
  const recommendedAmounts = [100, 200, 500, 1000, 5000, 10000];
     ...
     const serverRespo = (resp) => {
      ...
      if (resp.success) {
        ...
        setStatusDetails({
            "Status" : resp?.data?.trx?.status,
            "Amount" : resp?.data?.trx?.amount,
            "Network" : resp?.data?.trx?.network,
            "Phone Number" : resp?.data?.trx?.phone_number,
            "Date & Time " : resp?.data?.trx?.trx_date,
            "Trx Id " : resp?.data?.trx?.id,
        })
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-100 flex flex-col items-center">
      <div className="bg-white shadow-2xl rounded-2xl w-full max-w-3xl p-6 animate-fadeIn relative">
        ...
        <div className="mb-4  ">
          ...
          <span className="flex">
             <input type="tel" placeholder="Custom amount (<₦ 10,000)" value={amount} onChange={(e) => setAmountTyped(e.target.value)} className="w-full px-4 py-2 border  rounded-xl bg-gray-50 outline-none focus:border-blue-400" />
                <button onClick={() => handleBuy(amount)} className="ml-3 px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition !rounded-button whitespace-nowrap">Buy Airtime</button>
          </span>
        </div>
        ...
        {showStatus && <TrxStatus close={setShowStatus} trxDetails={statusDetails}/>}
        {showPins && (
