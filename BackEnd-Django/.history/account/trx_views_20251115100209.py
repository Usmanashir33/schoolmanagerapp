...
from core.trx_recorders import createAirtimeOrDataTrx,createWithdrawalTrx,createInternalTrx
...
class RecentReciepientsView(APIView):
    def get(self, request):
        try:
            ...
            data = MiniUserSerializer(distinct_users ,many=True).data
...
class AirtimeOrDataPurchaseView(APIView):
    permission_classes = [CheckTransectionPermission,]
    def post(self, request):
        # try :
            user = request.user
            network = request["network"] or None
            phone_number = requestk["phone_number"] or None
            amount = request.data.get("amount", None )
            payment_pin = request.data.get("payment_pin", None )
            data_plane = request.data.get("data_plane", None )
            transection_type = request.data.get("transection_type", None )
            TOTAL_AMOUNT = int(amount) + 0 

            # validate pin 
            if not user.checkPaymentPin(payment_pin) :
                return Response({"error":"wronge pin"}, status=status.HTTP_200_OK)
            
            # validate account balance 
            if not float(user.account.account_balance) >= float((TOTAL_AMOUNT)) :
                return Response(
                    {"error": "Insufficient balance"},
                    status=status.HTTP_200_OK)
                
            # process charges if its data 
            if transection_type.lower() == 'data' :
                pass
            # debit and credit here 
            user.account.debit(float(TOTAL_AMOUNT)) #user debited.
            # call the API here to process airtime or data purchase
            ...
            trx_data = createAirtimeOrDataTrx(request,data)

            return Response({
                "success" : f"{transection_type} purchase successfull",
                "transaction_data" : trx_data
