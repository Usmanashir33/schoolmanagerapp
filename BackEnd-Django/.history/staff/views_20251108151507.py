...
from authUser.models import StaffPins
        ...
        def put(self, request, user_id):
            try :
                ...
                if hasattr(user, 'staffpins'):
                    pin = user.staffpins.checkStaffPaymentPin(pins)
                else:
                    # handle missing pin
                    pin = None
                if not pin :
...
class StaffPinUpdateView(APIView):
    ...
    def post(self, request):
        try:
            ...
            else : #updating the staff pin  or setting new one 
                if staff_pin == None or len(staff_pin) == 0:
                    return Response({'error': 'pins is required.'}, status=status.HTTP_200_OK)
                if not user.checkVerificationCode(staff_otp) :
                    # for securoty recycle the otp 
                    email_verification_code = generate_verification_code() 
                    user.setVerificationCodeemail_verification_code 
                    user.save()
                    return Response({'error': 'invalid OTP.'}, status=status.HTTP_200_OK)
                    
                if hasattr(user, 'staffpins'): # already has set 
                    user.staffpins.setStaffPaymentPin(staff_pin)
                    user.staffpins.save()
                    return Response({'success': 'pins updated successfully.'}, status=status.HTTP_200_OK)
                else :
                    newpin = StaffPins.objects.create(
                        # pins = staff_pin,
                        user = user
                    )
                    newpin.save()
                    newpin.setStaffPaymentPin(staff_pin) 
                    try :
                        html_content = generate_otp_email(user.username,email_verification_code)
                        send_html_email.delay(
                            subject="🔐 Your Fentech OTP Code",
                        ...
                        pass
                    return Response({'success': 'pins set successfully.'}, status=status.HTTP_200_OK)
