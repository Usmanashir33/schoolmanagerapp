...
class StaffPinUpdateView(APIView):
    ...
    def post(self, request):
        try:
            user = request.user
            new_staff_pin,requst_type:"reset_staff_pin"
            new_pins = request.data.get('new_staff_pin')
            confirm_pins = request.data.get('confirm_pins')
            current_pins = request.data.get('current_pins')
            ...
            if hasattr(user, 'staffpins'):
                ...
                # handle missing pin
                pin = None
            if not pin or not current_pins == pin :
                    return Response({'error': 'wronge pins.'}, status=status.HTTP_200_OK)
            
            if new_pins != confirm_pins :
                return Response({'error': 'pins do not match.'}, status=status.HTTP_200_OK)
            
            if hasattr(user, 'staffpins'):
                user.staffpins.pins = new_pins
                user.staffpins.save()
            else:
                # create staff pins record if it doesn't exist
                from staff.models import StaffPins
                StaffPins.objects.create(user=user, pins=new_pins)
