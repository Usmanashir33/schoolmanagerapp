    ...
    def post(self, request):
        # try:
            ...
            if user.email ==  email :
                    ...
                    user.setVerificationCode(email_verification_code) 
                    user.save()
            ...
            if not usernamefound : #username not selected
                if not emailfound  and not phone_number_found:# username not selected
                    if len(password) >= 8 and (password == password2):
                            ...
                            newuser.setVerificationCode(email_verification_code)
                            newuser.save()
...
class VerifyWithEmailOtp(APIView):
    def post(self, request):
        # try:
            ...
            if user.email ==  email :
                    email_verification_code = generate_verification_code() 
                    user.setVerificationCode(str(email_verification_code)) 
                    user.save()
                    # we send email with email_verification_code
                    # send email here 
                    try:
...
class RegisterVerifyView(APIView):
    ...
    def post(self, request, format=None):
        # try:
            ...
            try :
                user = User.objects.get(email = email)
                authenticated = user.checkVerificationCode(email_verification_code)
            except :
                user = None
            ...
            if user and authenticated and not user.email_varified :
                ...
                user.setVerificationCode(generate_verification_code())
                user.save()
    ...
    def post(self, request, *args, **kwargs):
        # try :
            ...
            if user.log_with_otp and not otp : #we need otp  its otp request 
                ...
                user.setVerificationCode(generated_otp)
                user.save()
                ...
                if user.checkVerificationCode(otp): # otp verified
                    user.setVerificationCode(generate_verification_code())
                    user.save()
                    # send login message 
                    try:
...
class PasswordChangeView(APIView):
    ...
    def post(self, request,format=None):
        try:
            ...
            if mode == 'reset' :
                if (not user.checkVerificationCode(otp)) :
                    return Response({"error":"invalid otp"},status=status.HTTP_200_OK)
            else : #mode == change 
...
class PasswordResetView(APIView):
    ...
    def post(self, request,format=None):
        try:
            ...
            if (user.checkVerificationCode(email_otp)) :
                if user.checkPaymentPin(user_pin) :
                    if (password1 == password2) :
                        user.set_password(password1)
                        user.setVerificationCode(generate_verification_code()) 
                        user.save()
...
class PasswordForgetView(APIView):
    ...
    def post(self, request,format=None):
        try:
            ...
            if (user.checkVerificationCode( otp)) :
                    if (password1 == password2) :
                        user.set_password(password1)
                        user.setVerificationCode(generate_verification_code()) 
                        user.save()
