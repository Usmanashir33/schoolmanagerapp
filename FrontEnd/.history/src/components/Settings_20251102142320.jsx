...
import ToggleLoginWithOTP from "./ToggleLoginWithOTP";
    


const Settings = () => {
  const navigate = useNavigate();
  const [manageOtpLogin,setManageOtpLogin] = useState(!false);
  ...
  const handle2FAChange = () => {
    ...
    // display pin model to enable or disable 2fa and pass the status updater 
  ...
  const LogoutPanel = () => {
    return(
      <>
        { <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 w-lg ">
          <div className="bg-white rounded-xl p-4 w-[90%] max-w-md">
            <div> <h3 className="font-semibold text-gray-800">Confirm Logout</h3></div>
            <div className="flex justify-between gap-4 mt-4">
              <button type="button" className="bg-blue-600 text-white px-10 py-1  rounded-md w-full hover:opacity-70 flex gap-3 justify-center items-center"
                onClick={() => {setConfirmLogout(!true)}}>
                Cancel

              </button>
              <button type="button" className="bg-red-500 text-white px-10 py-1 rounded-md w-full hover:opacity-70 flex gap-3 justify-center items-center"
                onClick={() => { logout() }}> 
                Logout 
              </button>
            </div>
  ...
  // login user with otp status
  useEffect(() => {
   if (currentUser) {
    setTwoFactorEnabled(currentUser.log_with_otp);
   }
  }, [currentUser]);  
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex justify-center">
      ...
      {confirmLogout && <LogoutPanel closePopup={setConfirmLogout}/>}
      {manageOtpLogin && <ToggleLoginWithOTP closePopup={setManageOtpLogin} />}
      ...
      <div className="w-full bg-white shadow-sm transition-all duration-300 hover:shadow-md">
        <div className="p-4 ">
            ...
            <div className="bg-white rounded-lg shadow-sm">
              ...
              <div className="flex items-center justify-between p-3 border-b border-gray-100 transition-colors duration-200 hover:bg-gray-50">
                ...
                <div
                  ...
                  onClick={() => setManageOtpLogin(!twoFactorEnabled)}
                >
                  <div
          ...
          <div className="mt-8 mb-4"> 
            <button className="w-full py-3 bg-red-500 text-white rounded-lg hover:bg-opacity-80 transition-all duration-300 hover:shadow-md transform hover:-translate-y-1 font-medium rounded-lg !rounded-button whitespace-nowrap cursor-pointer transition-all duration-300 hover:shadow-md transform hover:-translate-y-1"
            onClick={() => {setConfirmLogout(true)}}>
