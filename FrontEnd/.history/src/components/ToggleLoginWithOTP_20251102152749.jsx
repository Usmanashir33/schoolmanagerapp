...
const ToggleLoginWithOTP = ({closePopup}) => {
  ...
  const {isLoading,setSuccess} = useContext(uiContext)
  ...
  const updateUiFromServer = (data) => {
    let updated_data= data.user
    setCurrentUser(updated_data)
    setSuccess(d)
    setShowPins(false);
  ...
  const handleProceedToServer = (pin) => {
    ...
    let url = `/authuser/manage-user-login/`
    let data = {
        payment_pin:pin,
        log_with_otp:!otpEnabled
    }
    sendArbitRequest(url,"POST",data,updateUiFromServer,true,false)
  };
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 w-[90%] max-w-md animate-fadeIn relative">
            ...
            <div className="flex items-center justify-between border rounded-lg p-3 bg-gray-50">
              ...
              <div className="flex justify-between gap-4 items-center">
                {isLoading && <RiLoader2Fill strokeWidth={0.5} className="h-5 w-5 text-blue-600  animate-spin" />}
                {!isLoading && <button
