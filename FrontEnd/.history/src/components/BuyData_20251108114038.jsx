...
export default function DataSellingPage() {
  ...
  const verifyPins = (pins) => {
    // Here you would typically verify the PINs with your backend or authentication service
    // For this example, we'll just log them to the console and close the PIN modal
    alert( pins);
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-100 flex flex-col items-center">
      ...
      <div className="bg-white shadow-2xl rounded-2xl w-full max-w-3xl p-6 animate-fadeIn relative">
        ...
        {/* show pins here  */}
        {showPins && <Pins close={setShowPins} triggerFunc={verifyPins} />}
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl shadow-2xl p-6 w-[90%] max-w-sm relative animate-fadeIn">
              ...
              <div className="flex justify-center gap-4 mt-5">
                ...
                <button onClick={() => { 
                    alert(`Purchased ${confirmation.size} on ${confirmation.number} (${confirmation.network}) for ₦${confirmation.price}`); setConfirmation(null); }} className="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700">Confirm</button>
              </div>
            </div>
          </div>
        )}
